...
impl FraudIndex {
    ...
    /// Returns the fraud neighbor count (0–5) for query vector `q` using k-NN
    /// with k=5.
    ///
    /// Scans all reference rows, maintaining a sorted top-5 by distance.
    /// Uses insertion sort so `neighbors[4]` is always the worst (largest)
    /// distance — the bound tightens monotonically, skipping rows early.
    pub fn search(&self, q: &[f32; 14]) -> u8 {
        let q_cont = query_cont_bytes(q);
        let pd = PartialDists::compute(q);

        // Sorted ascending by distance; neighbors[4] is always the worst.
        // Initialized to (MAX, false) so the first 5 candidates always insert.
        ...
        let (q_simd, zero_lanes_mask) = unsafe {
            ...
            // Mask bytes 12-15 to zero: packed bits and label must not contribute
            // to the continuous distance sum.
        ...
        for row in &self.rows {
            ...
            // Skip early if this row is not better than the current worst neighbor.
            if dist >= neighbors[4].0 {
                continue;
            }

            let is_fraud = row[15] != 0;

            // Insertion sort: shift right from the tail until we find the
            // correct position, then insert. neighbors[4].0 auto-updates.
            let mut pos = 4;
            while pos > 0 && dist < neighbors[pos - 1].0 {
                neighbors[pos] = neighbors[pos - 1];
                pos -= 1;
            }
            neighbors[pos] = (dist, is_fraud);
        }

        neighbors.iter().filter(|(_, f)| *f).count() as u8
    }
}
...
mod tests {
    ...
    #[test]
    fn search_insertion_sort_order_is_correct() {
        // Feed 7 rows with distinct distances (via different dim 0 values).
        // Only the 5 closest should be in the top-5.
        // Rows sorted by distance from ZERO: close (score 1.0), mid (score 0.0),
        // then 5 more legit rows even farther away.
        let very_close_fraud = {
            let mut v = ZERO;
            v[0] = 0.01;
            v
        };
        ...
        // 3 fraud at very_close, 2 legit at close, 2 legit at far → top-5 = 3 fraud + 2 close legit
        let idx = make_index(&[
            (very_close_fraud, true),
            (very_close_fraud, true),
            (very_close_fraud, true),
            (close_legit, false),
            (close_legit, false),
